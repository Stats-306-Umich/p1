...
```{r}
gas <- read_csv("./data/dutch_ttf_natural_gas.csv", col_types = cols(Date = col_date(format = "%Y-%m-%d")))
```
...
daily %>%
  ...
  geom_density()
```
From the marginal distribution of the consumption power variable, we can see that the distribution is right-skewed. This means that the majority of the data points are concentrated on the lower end of the consumption power scale. 
...
daily %>%
  ...
  geom_point()
```
From the joint distibution of the consumption power and temperature, we can see that there is a negative correlation between the two variables. This means that as the temperature increases, the consumption power decreases. 

```{r}
# Conditional mean
daily %>%
  group_by(weekday) %>%
  summarise(mean_power = mean(consumption_power)) %>%
  ggplot(aes(x = weekday, y = mean_power)) +
  geom_point()
```
From the conditional mean plot, we can see that the consumption power is highest on the weekends and lowest on the weekdays. This is likely due to the increased use of electricity on the weekends when people are home. 
...
```
From the facet wrap plot, we can see that the consumption power is lower during the Christmas period than during the rest of the year. This is likely due to the fact that people are away from home during the Christmas period.
...
daily %>%
  ...
  group_by(month) %>%
  summarise(mean_power = mean(consumption_power)) %>%
  ggplot(aes(x = month, y = mean_power)) +
geom_point() +
coord_polar()
  

```
From the plot of the seasonal effects on the consumption power, it is clear that the consumption power is highest in the summer months and lowest in the winter months. This is likely due to the increased use of air conditioning in the summer months.
...
daily %>% 
  group_by(holiday) %>% 
  summarise(stdev = sd(wages)) %>% 
  ggplot(aes(x = weekday, y = mean_wage)) + 
  geom_point() +

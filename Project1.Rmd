...
In our project, we began by analyzing the dimesions of the "daily" and "gas" datasets. We then summarized the data by calculating the mean, standard deviation, minimum, median, and maximum values for the three types of natural gas consumption. We analyzed further the number of weeks covered, the percentage change in consumption variables, and the proportion of holidays. We created plots to visualize the trends in natural gas prices and consumption over time during the crisis, and we explored missing values in the dataset. We fulfilled the objectives of this project by looking at how exactly the usage of power or heating varied with the holiday season during the crisis, months, and temperature. In our exploration of the consumption power as it varies with the months in a year, we found that consumption power is highest in the winter months by a significant margin, which contridicts our result of its mean being lower during the christmas period than when it is not christmas. Additionally, we confirmed that temperature has a negative correlative relationship with consumption power, implying causation supported by our conclusions from the seasonal data. Interestingly enough, we also found that heating indexes are lowest on weekends on average. First, we looked at  investigated solar radiation's marginal distribution and its relationship with temperature. Our findings indicate that there are significant differences in natural gas consumption patterns based on various factors such as temperature and the month. This research is important as it provides insights into energy consumption trends during a crisis period and can inform future energy policies.
